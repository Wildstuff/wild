...
[`data/regenerate-seed.py`](./data/regenerate-seed.py), run it, and move
`window.start` in `apps/liquidity-report.yaml` to the first Monday after. An
absolute window and a fixed data cut are the same maintenance fact seen twice.
...
invoices are open, how much is already overdue, and the next 13 weeks in and
out — then the accounts and the bank movements those figures fold from. One
page, five figures, no schema tour. Start here when you want to see what the
tribe *knows*.
...
invoices' `due_date`. `window.start` is ABSOLUTE (the renderer carries no
clock), so it is pinned to the Monday this bundle's sample data was cut for —
re-cut the data, move the start, or the window renders empty.
