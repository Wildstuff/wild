...
# `effective_status: overdue` on each past-due open receivable, and the
# `change: {field: effective_status, becomes: overdue}` trigger fires this
...
tick wrote `effective_status: overdue`; occasionally by the chief's judged
