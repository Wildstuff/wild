...
bootstrap; see [`docs/embed-adapters.md`](../../docs/embed-adapters.md));
...
2. Calls `wild:ai/embed.embed(...)` with `model: none` and a
   ...
   `~/.wild/embed-adapters.yaml` (`embed_routing.default` or
...
# import wild:ai/embed@0.3.0;
# export wild:messaging/handler@0.1.0;
```

> First build after a fresh clone needs the external WIT vendored:
> `cargo run -p xtask -- wit-sync`.

## Deploy

A working embed adapter must be wired first — see
[`docs/embed-adapters.md` § Operator install walkthrough](../../docs/embed-adapters.md#operator-install-walkthrough)
(`ollama-embed` + a local `ollama serve`).
...
[`crates/runtime/wild-host/tests/embed_consumer_e2e.rs`](../../crates/runtime/wild-host/tests/embed_consumer_e2e.rs)
...
- [`docs/embed-adapters.md` § Calling embed from a component](../../docs/embed-adapters.md#calling-embed-from-a-component)
  — the call ergonomics + the `caller-context` umbrella-vs-adapter
  distinction.
- [`plugins/workers/ai-worker/`](../../plugins/workers/ai-worker/) — the
  fuller workload-flavor reference (chat + tool-loop + files).
