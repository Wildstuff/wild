...
| `EncryptedFileBackend` (`file`) | `$WILD_HOME/system/secrets.enc` (override `WILD_SECRETS_FILE_PATH`), AES-256-GCM, master key from `WILD_SECRETS_KEY` (64-hex) else the generated `system/secrets.key` sibling, atomic writes, mode 0600 | yes | Joins the chain whenever `file` is in the order and a key resolves — the DEFAULT leader on any host without an OS credential store. Dropped with a WARN only when the key can be neither read nor generated. |
...
- **EncryptedFile behind it on a desktop, and not to store secrets.**
  The master key is only resolved when the chain lists `file` (see
...
`dashboard-token`, `forge/install_key`, and `secrets.key` when the file
