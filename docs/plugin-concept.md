...
of how the host talks to it. The five flavors split into two families
(ADR-0062 pinned this set; interaction transports are a layer, not a
sixth flavor):
...
|---|---|---|---|
| **llm-adapter** | `wild:llm-adapter@0.1.0` | `wild:ai/chat` | `openrouter`, `anthropic-cli`, `openai` |
| **storage-adapter** | `wild:storage-adapter@0.1.0` | `wild:files` blob backend | *(seam defined; no plugin bundled today)* |
| **embed-adapter** | `wild:embed-adapter@0.1.0` | `wild:ai/embed` | `ollama` |
...
Every Tier-2 plugin exports `wild:plugin-meta@0.2.0` — the minimum
...
`wild:ai@0.3.0`, `wild:files@0.4.0`). The imports a world declares are
...
The split exists because the sidecar is signed content (cosign-verify
checks the digest): if config lived in the sidecar, every
...
| `default_pool_size` | provider-flavor instance-pool hint |
...
is `~/.wild/profiles/<name>/`; operator data sits at the root, runtime
...
| Publisher keys + tier overrides | `~/.wild/plugin-trust.json` (**system-wide**) | `PluginTrustStore::default_path()` |
...
- Nothing lives in a top-level `~/.wild/plugins/` folder. Per
...
|---|---|---|
| **Verified** | Built-in bootstrap rows, or a cosign signature against Wildstuff's key | The full set its `requires` declares |
| **Community** | Signed by a known, operator-allowlisted publisher | A subset — no privileged caps (e.g. no secrets-write, no bundle admin) |
| **Unknown** | Locally added (`wild plugin add ./file.wasm`) or unsigned | The minimum-viable subset; the operator widens it per-plugin via `wild plugin grant <slug> <cap>` |

A plugin with no explicit tier defaults to **Unknown**, so the plan
renderer fails safe.
...
future meaning is an org-local cosign key for Forge-built plugins,
...
>
> - **Concept:** [`plugin-trust.md`](plugin-trust.md) (operator schema + CLI) · ADR-0045 (out-of-tree metadata + trust) · `bootstrap-and-default-inventory.md` (the default-shipped inventory + tiers).
> - **In code:** `crates/runtime/wild-plugins/src/trust.rs` (`PluginTrustStore`) · the load-time gate in `crates/runtime/wild-host/src/`.
...
| `update` | install | replace bytes + sidecar for an installed slug |
| `remove` | install | delete the sidecar + wasm |
...
| Wasmtime, the cosign-verify path | Platform + trust bootstrap |
