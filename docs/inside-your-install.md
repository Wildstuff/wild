...
> Everything The Wild knows lives in plain files under `~/.wild/` — no
...
└── profiles/<name>/              ← one profile = one self-contained world
    ...
    ├── 🔎 ideas/ tribes/ …        ← your Tribes' state — inspect freely
...
flowchart TD
    ...
    Root --> State["🔎 you inspect<br/>ideas/ · tribes/ · decisions/<br/>tribes/&lt;slug&gt;/sessions/ · tribes/&lt;slug&gt;/files/"]
...
| Path | What's there |
|---|---|
| `ideas/ID-<hex>/` | A pinned idea (a dormant Tribe): `meta.yaml`, `shape.yaml`, `spine.md`, `extends/`. |
...
**without** touching your `ideas/` / `tribes/` / `decisions/` /
