...
truth (a stray sibling file fails `xtask pre-push`). The root file
also declares the `wasm32-wasip{1,2}` targets the build needs.

Required cargo extensions:

```sh
cargo install cargo-component   # builds wasm32-wasip2 components
cargo install cargo-nextest     # 3× faster test runner used by xtask
cargo install --locked wkg      # WebAssembly registry CLI (external WIT)
```
...
are not committed as text-WIT. They're vendored as binary `.wasm`
under `wit/external/` keyed off `scripts/wit-external.lock`. Run
...
step every standalone plugin build fails with `package wasi:* not
found`.

To force-refresh after a registry republish:
...
```
~/.wild/profiles/<profile>/system/plugin-cache/
...
Never use `cargo test --workspace` while iterating — it builds and
runs ~1500 tests on a single thread (~27min cold). The xtask
mirrors CI's per-crate matrix: 1–3 crates in <2min for a typical
diff. Mandatory before `git push`: `xtask pre-push` enforces
fmt + ADR-0004 lockdown + affected tests in CI's order.
...
| Tier-2 | Wasm component on disk, exporting a `wild:*` capability | `~/.wild/profiles/<profile>/system/plugin-cache/<slug>.{json,wasm}` | `wire_p1_component_plugins` (sync) or `wire_tier2_llm_adapters` (async post-start) |
...
one like any Tier-2 component (`plugins/channels/telegram-channel/` is the
reference, outbound `deliver` + `capabilities`); see the *Channels* section in
...
in-tree connectors (`folder-connector`, `web-connector`) and adapters
(`csv-parser`, `json-parser`, `pdf-parser`, `brave-search`,
`math-tools`, `http-fetcher`) — shares the SAME boilerplate: a
...
below. See `plugins/tools/folder-connector/src/lib.rs` for the minimal
shape. The macros are `macro_rules!` (not a proc-macro) on purpose: the
...
`wit/external/` is gitignored. `scripts/wit-external.lock` plus
`xtask wit-sync` are the reproduction record.
...
| `verified-local` | Same as `verified` for sidecars built or pulled by trusted local tooling (operator-curated). |
| `community` | Blocklist: no `wild:secrets/store`/`admin`, no `wild:bundles/admin`, no `wild:plugin-storage/*`. Reads are fine. |
| `unknown` | Allowlist: `wasi:http/outgoing-handler`, `wild:blobs/read`, `wild:messaging/consumer` only. |

Capability overrides are additive across all tiers — every entry
in `~/.wild/plugin-trust.json` `capability_overrides` for a slug
...
not a string: `{ "tier": "verified-local" }`, never `"trust": "verified-local"`.
...
Reference: `plugins/llm/anthropic-cli/`. This plugin imports the
...
```sh
mkdir -p ~/.wild/profiles/<profile>/system/plugin-cache
cp target/wasm32-wasip2/release/anthropic_cli.wasm \
   ~/.wild/profiles/<profile>/system/plugin-cache/anthropic-cli-1.1.0.wasm
cat > ~/.wild/profiles/<profile>/system/plugin-cache/anthropic-cli.json <<EOF
{
  "slug": "anthropic-cli",
  "version": "1.1.0",
  "source": { "type": "local", "path": "anthropic-cli-1.1.0.wasm" },
...
allowlist at `~/.wild/profiles/<profile>/cli-binaries.yaml` must
...
Reference: `plugins/tools/http-fetcher/`. Shorter walkthrough — the
...
Elder speaking to an accountant, not an engineer. Precedents:
...
- `WARN trust gate denied capability` → the plugin's `requires:`
  list contains a cap not allowed for its tier. Either bump the
  tier (operator decision, see §4) or grant the cap explicitly:
  `wild plugin grant <slug> <cap@ver>`.
