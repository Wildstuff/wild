...
- **The bundle is self-contained** — the CLI, the daemon and the NATS server
  are siblings inside it, so a `.app` install needs none of the paths below.
  `Contents/MacOS/wild` is the CLI if you want it on your `PATH`.
...
built for it since, so the installer refuses an Intel Mac up front —
the alternative is a 404 on a URL the operator never typed, which
reads as a broken download rather than a platform decision. The way
on is a source build:
...
**Two things it does not do.** It does not install a signed package —
there is no Authenticode certificate yet, so Windows SmartScreen warns
on first run. And it does not ship the desktop app's own background
daemon (`wild-appd` is macOS-only). What it does do is verify the
download's sha256 before unpacking anything, and refuse the install
