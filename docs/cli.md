...
wild doctor                # walks every layer, prints pass/fail
...
wild tribe apply <bundle-dir>            # apply a bundle DORMANT by default: manifest + blueprint + ontology/seeds.yaml pinned + sample-data ingested, but the chief + connectors do NOT run until activate
wild tribe apply <bundle-dir> --start    # turnkey: apply AND deploy-and-run immediately (the pre-dormant-default behaviour)
...
wild plugin update <slug> --tier verified  # promote trust tier
```
...
```bash
wild elder dump-prompt --surface mcp --mode intake   # exact composed host-Elder
                                                     #   prompt (base + intake.md +
                                                     #   profile ELDER.md), to stdout
wild elder dump-prompt --surface mcp --mode operate  # same, Operate mode fragment
wild elder dump-prompt --surface root                # pointer: root chat is the chief
```
...
live" hack. The `mcp` surface (host-Elder / `CliElderBackend` + MCP) is
composed offline from the single source in `common::elder_mode` and is
authoritative; `--surface root` / `--surface tribe:<id>` are the CHAT
surfaces served by the chief (composed in the chief wasm guest), so they
print a pointer to `assets/prompts/chief-default/cycle.md` rather than a
wrong answer. The banner goes to stderr, so `… --surface mcp > out.md`
captures only the prompt body. See
`docs/prompt-layers.md` for the surface→files map.
...
the first item. `--sample` additionally maps a sample row through the same
probe the intake pipeline uses — the verify-before-done completion proof
(ADR-0152 D9): only a green probe WITH a successful sample means the
source is truly set up. Nothing is ingested or persisted. Use it to prove
a connector is reachable and correctly configured before scheduling intake
or after rotating credentials. Returns success/failure, item count,
first-item metadata, fault classification, and elapsed ms.
