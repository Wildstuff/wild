...
| `src/**`, `Cargo.toml`, `Cargo.lock` | the source, and the exact dependency resolution the build used |
...
oras pull ghcr.io/wildstuff/templates/slugify:0.1.0 -o pkg/
...
sig = open("pkg/slugify-0.1.0.sig", "rb").read()
VerifyKey(key).verify(open("pkg/slugify-0.1.0.tar.gz", "rb").read(), sig)
print("signature OK")
PY
```
